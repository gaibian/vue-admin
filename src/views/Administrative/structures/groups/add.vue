              ...
              <div v-for="(item,index) in nodes" :key="index">
                ...
                <div v-for="(childItem,index) in item.child" :key="index">
                  ...
                  <div class="p-l-40 bor-b-ccc bg-gra">
                    <el-checkbox v-for="(grandChildItem,index) in childItem.child" v-model="grandChildItem.check" :key="index" @change="selectActionRule(grandChildItem, childItem, item)">{{grandChildItem.else}}</el-checkbox>
                  </div>
                </div>
              </div>
            </div>
...
<script>
  ...
  import { addGroupSubmit,getRulesData,groupsListRender } from '@/api/groupsList'
  export default {
    mixins: [formMixin],
    data() {
      return {
      ...
      add(form) {
        ...
        this.$refs[form].validate((valid) => {
          if (valid) {
            ...
            addGroupSubmit(this.form).then(res=>{
